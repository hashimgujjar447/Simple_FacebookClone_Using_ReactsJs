...
function SignUpComponent() {
  const { addUser, users } = useFacebookContext();
  ...
  const yearsArray = [];
  for (let year = 1905; year <= 2025; year++) {
    yearsArray.push(year);
  }
  const [FirstName, setFirstName] = useState("");
  const [lastName, setLastName] = useState("");
  const [password, setPassword] = useState("");
  const [dayValue, setDayValue] = useState(daysInMonth[0]);
  const [yearValue, setYearValue] = useState(yearsArray[0]);
  const [monthValue, setMonthValue] = useState(monthsArray[0]);
  ...
  function handleSignUp() {
    ...
    addUser({ username: user, password: password });
  ...
  useEffect(() => {
    console.log(users);
    localStorage.setItem("UsersAvailable", JSON.stringify(users));
  }, [users]);
  ...
  return (
    <div className="flex items-center flex-col px-4 sm:px-6 lg:px-8">
      ...
      <div className="bg-white w-full max-w-md rounded-lg shadow-md py-3 mb-12">
        <h1 className="text-3xl font-medium text-center">Create new Account</h1>
        <p className="text-center text-[#606770]">It's quick and easy.</p>
        <hr className="border-[#dadde1] mt-4 mb-4" />

        <form className="p-2 px-4 gap-3" onSubmit={(e) => e.preventDefault()}>
          <div className="flex flex-col sm:flex-row items-center gap-3">
            <input
              type="text"
              className="w-full sm:w-1/2 border-[#ccd0d5] border-1 p-2 rounded-lg"
          ...
          <div>
            <p className="text-[#606770] text-sm py-3">Date of birth</p>
            <div className="flex flex-col sm:flex-row gap-3">
              <select
                value={dayValue}
                onChange={(e) => setDayValue(e.target.value)}
                className="border-[#ccd0d5] border-1 p-2 rounded-lg w-full sm:w-1/3"
              >
                {daysInMonth &&
                  daysInMonth.map((day) => <option key={day}>{day}</option>)}
              </select>
              <select
                value={monthValue}
              ...
              >
                {yearsArray &&
                  yearsArray.map((year) => <option key={year}>{year}</option>)}
              </select>
            </div>
            <div>
              <p className="text-[#606770] text-sm py-3">Gender</p>
              <div className="flex flex-col sm:flex-row gap-2">
                <div
                  onClick={() => setSelectedGender("Male")}
                  className={`flex items-center gap-2 justify-between border-[#ccd0d5] border-1 p-2 rounded-lg w-full sm:w-1/3 ${
                    selectedGender === "Male" ? "bg-blue-100" : ""
                  }`}
                >
                  <p>Male</p>
                  <input type="radio" checked={selectedGender === "Male"} />
                </div>
                <div
                  onClick={() => setSelectedGender("Female")}
                  className={`flex items-center gap-2 justify-between border-[#ccd0d5] border-1 p-2 rounded-lg w-full sm:w-1/3 ${
                    selectedGender === "Female" ? "bg-blue-100" : ""
                  }`}
                >
                  <p>Female</p>
                  <input type="radio" checked={selectedGender === "Female"} />
                </div>
                <div
                  onClick={() => setSelectedGender("Custom")}
                  className={`flex items-center justify-between gap-2 border-[#ccd0d5] border-1 p-2 rounded-lg w-full sm:w-1/3 ${
                    selectedGender === "Custom" ? "bg-blue-100" : ""
                  }`}
                >
                  <p>Custom</p>
                  <input type="radio" checked={selectedGender === "Custom"} />
                </div>
              </div>
            </div>
            <div className="mt-3">
              <input
                type="text"
                className="w-full border-[#ccd0d5] border-1 p-2 rounded-lg"
            ...
            <div>
              <p className="text-xs text-[#606770] mt-6 mb-3">
                People who use our service may have uploaded your contact
                information to Facebook.{" "}
                <span className="text-blue-400">Learn more.</span>
              </p>
              <p className="text-xs text-[#606770] mt-3 mb-3">
                By clicking Sign Up, you agree to our{" "}
                <span className="text-blue-400">Terms,Privacy Policy</span> and{" "}
                <span className="text-blue-400">Cookies Policy</span>. You may
            ...
            <div className="flex justify-center items-center mt-6">
              <button
                ...
                className="text-white font-bold text-xl p-2 px-9 rounded-lg bg-[#119f16]"
              >
                Sign Up
              </button>
            </div>
            <Link to={"/login"}>
              <p className="text-[#1877f2] text-center text-lg font-medium pt-4 pb-4">
